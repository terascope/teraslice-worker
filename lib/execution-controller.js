...
// TODO: Add _adjustSlicerQueueLength, _allSlicesProcessed
...
class ExecutionController {
    ...
    async runOnce() {
        ...
        if (this.scheduler && !this.allSlicersDone
            && (this.slicerQueue.size() < this.queueLength)) {
    ...
    async setFailingStatus() {
        const exId = this.jobConfig.ex_id;

        const errMsg = `slicer: ${exId} has encountered a processing_error`;
        this.logger.error(errMsg);
        // FIXME: const executionStats = executionAnalytics.getAnalytics();
        // const errorMeta = this.stores.exStore.executionMetaData(executionStats, errMsg);
        // this.stores.exStore.setStatus(exId, 'failing', errorMeta);
    }

    async slicerCompleted() {
        const exId = this.jobConfig.ex_id;
        const totalSlicers = this.slicers.length;

        this.slicerDone += 1;

        this.logger.info(`a slicer for execution: ${exId} has completed its range`);

        if (this.slicerDone === totalSlicers) {
            this.events.emit('slicers:finished');
            this.allSlicersDone = true;
            // TODO
        }
    }
    ...
    _adjustSlicerQueueLength() {
        ...
        const clientsOnline = this.messaging.getClientCounts();
