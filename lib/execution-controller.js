...
class ExecutionController {
    ...
    async _doneProcessing() { // TODO: this is not complete
        const { events } = this;
    ...
    async _waitForSlicesToComplete() {
        ...
        const pendingSlices = this.messenger.pendingSlices();

        this.logger.trace(`worker queue: ${availableWorkers}, active clients ${connectedWorkers}, slicer queue: ${remainingSlices}/${pendingSlices}`);

        const workersCompleted = availableWorkers >= connectedWorkers;
        const slicesFinished = remainingSlices === 0;

        if (workersCompleted && slicesFinished && pendingSlices === 0) {
