...
class ExecutionController {
    ...
    async initialize() {
        ...
        this.logger.info(`Setting slicer queue length to ${this.queueLength}`);
    ...
    async slicerFailure(err) {
        ...
        const error = new WrapError(`slicer for ex ${this.exId} had an error, shutting down execution`, err);

        // FIXME: const executionStats = executionAnalytics.getAnalytics();
        // FIXME: const errorMeta = exStore.executionMetaData(executionStats, errMsg);
        this.logger.error(error.toString());

        // FIXME: exStore.setStatus(exId, 'failed', errorMeta)
        //     .then(() => {
        //         this.messenger.send({
        //             to: 'cluster_master',
