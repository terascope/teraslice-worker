 ...
 * @param {object} config              the teraslice worker configuration
 * @param {string} config.assignment   assignment must be worker
 * @param {string} config.ex_id        execution identifier
 * @param {string} config.job_id       job identifier
 * @param {number} config.slicer_port  slicer port to bind to
 * @param {object} config.job          job configuration
 * @param {boolean} useExecutionRunner use execution runner from teraslice
 *                                     instead of the new version
 */

class ExecutionController {
    constructor(context, config, useExecutionRunner) {
        validateConfig(config);
        const workerId = generateWorkerId(context);
        const logger = makeLogger(context, config, 'execution_controller');
        const events = context.apis.foundation.getSystemEvents();

        const port = config.slicer_port;
        ...
        this.job = makeJob(context, config, useExecutionRunner);

        this.executionAnalytics = new ExecutionAnalytics(context, config, this.messenger);
        ...
        this.config = config;
        this.exId = config.ex_id;
        this.jobId = config.job_id;
        this.collectAnalytics = this.config.job.analytics;
    ...
    async _slicerInit() {
        ...
        const maxRetries = get(this.config, 'job.max_retries', 3);
        const retryOptions = {
