...
class ExecutionController {
    constructor(context, executionContext) {
        ...
        this.slicerFailure = this.slicerFailure.bind(this);
    }
    ...
    async _slicerInit() {
        const {
            ...
            context,
            executionContext,
            executionStartingPoints
        } = this;

        // handle the case where recover should exit
        if (_.get(executionStartingPoints, '_exit')) {
            this.slicers = [() => null];
            return;
        }
    ...
    async _createSlices() {
        ...
        if (this.slicerQueue.size() < this.executionContext.queueLength) {
            await Promise.map(scheduler, slicerFn => slicerFn());
        }
    }

    async _registerSlicers() {
        this.executionStartingPoints = await this.engine.executionRecovery();
        await this.engine.registerSlicers(this.slicers);
    }
