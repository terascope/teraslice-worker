...
class ExecutionController {
    constructor(context, executionContext) {
        ...
        this.executionAnalytics = new ExecutionAnalytics(context, executionContext, this.clusterMasterClient);
    ...
    async initialize() {
        ...
        await this.clusterMasterClient.start();
        ...
        this.clusterMasterClient.on('cluster:execution:pause', async (msg) => {
            try {
                await this.pause();
            } catch (error) {
                this.logger.error('Unable to pause', error);
                await this.clusterMasterClient.respond(msg, { error });
                return;
            }

            await this.clusterMasterClient.respond(msg);
        });

        this.clusterMasterClient.on('cluster:execution:resume', async (msg) => {
            try {
                await this.resume();
            } catch (error) {
                this.logger.error('Unable to resume', error);
                await this.clusterMasterClient.respond(msg, { error });
                return;
            }
    ...
    async pause() {
        ...
        await new Promise((resolve) => {
            ...
            this.events.once('worker:paused', () => { resolve(); });
            this.isPaused = true;
        });

        this.logger.info(`execution ${exId} is paused`);
    }
    ...
    async _processSlices() {
        if (this.isPaused) {
            this.events.emit('worker:paused');
