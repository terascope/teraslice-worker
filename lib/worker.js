...
class Worker {
    ...
    async initialize() {
        ...
        this.stores = await Promise.props({
            stateStore: makeStateStore(context),
            analyticsStore: makeAnalyticsStore(context),
        });
    ...
    async start() {
        await this._waitAndProcessSlice();
    }
    ...
    async _waitAndProcessSlice() {
        ...
        try {
            msg = await this.messenger.onceWithTimeout('slicer:slice:new');
        } catch (err) {
            return this._waitAndProcessSlice();
        }
