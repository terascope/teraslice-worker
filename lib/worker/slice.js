...
const { logOpStats } = require('../utils');
...
class Slice extends TerasliceWorker {
    ...
    async _logAnalytics() {
        ...
        try {
            ...
            throw new TerasliceError('Failure to update analytics', _err);
        }
    }
    ...
    async _markCompleted(result) {
        ...
        try {
            ...
            throw new TerasliceError('Failure to update success state', _err);
        }
    ...
    async _markFailed(err) {
        ...
        try {
            ...
            throw new TerasliceError('Failure to failure update state', _err);
        }

        events.emit('slice:failure', slice);
        logger.error(`Slice failed processing: ${errMsg}, message: `, slice);
        const sliceError = new TerasliceError('Slice failed processing', err);
