...
module.exports = function makeEngine(controller) {
    ...
    function registerSlicers(slicersArray, isRecovery) {
        ...
        // FIXME: executionAnalytics.set('slicers', totalSlicers);
    ...
    function _checkAndUpdateExecutionState() {
        ...
        return () => {
            ...
            watchDogSet = true;
            // FIXME: const analyticsData = executionAnalytics.getAnalytics();
    ...
    function _createSlices(slicerFn, slicerId, lifecycle) {
        ...
        return function createSliceFn() {
            if (!isProcessing && !hasCompleted) {
                ...
                Promise.resolve()
                    .then(() => slicerFn())
                    .then(async (sliceRequest) => {
                        // not null or undefined
                        if (sliceRequest != null) {
                            if (_.isArray(sliceRequest)) {
                                logger.warn(`slicer for execution: ${exId} is subslicing by key`);
                                // FIXME: executionAnalytics.increment('subslice_by_key');
                            }
                            slicerOrder = await controller.allocateSlice(sliceRequest, slicerId, slicerOrder);
                        } else if (isOnce) {
