...
class ExecutionControllerMessenger extends MessageEmitter {
    ...
    sendWithResponse(workerId, eventName, payload, options) {
        if (!this.workers[workerId]) {
            throw new Error(`Cannot send message to worker ${workerId}`);
        }
        const { socket } = this.workers[workerId];
        return sendMessageWithResponse(socket, eventName, payload, options);
    }

    sendNewSlice(workerId, slice) {
        return this.sendWithResponse(workerId, 'slicer:slice:new', slice, {
