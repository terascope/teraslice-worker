...
class Job {
    ...
    async _loadOperation(opName) {
        ...
        if (!isString(opName)) {
            throw new TerasliceError('please verify that ops_directory in config and _op for each job operations are strings');
        }
        ...
        try {
            ...
            const error = new TerasliceError(`Failed to module by path: ${opName}`, _error);
            try {
                ...
                const wrappedError = new TerasliceError(error.toString(), err);
                throw new TerasliceError(`Failed to module: ${opName}`, wrappedError);
            }
        }
    }
}
