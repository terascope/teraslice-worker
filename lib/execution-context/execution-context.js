...
class ExectionContext {
    constructor(context, executionContext) {
        ...
        this._operations = _.get(executionContext.job, 'operations', []);
    ...
    _getOpConfig(name) {
        return _.find(this._operations, { _op: name });
    }

    async _initializeSlicer() {
        const opConfig = _.first(this._operations);
    ...
    async _initializeOperations() {
        ...
        this.queue = await Promise.map(this._operations, async (opConfig, index) => {
